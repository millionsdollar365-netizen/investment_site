        ...
        <div class="card-body">
            <form id="planForm" style="display:flex;flex-direction:column;gap:.75rem">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.75rem">
                    <div><label style="font-size:.78rem;font-weight:600;color:var(--argon-dark);display:block;margin-bottom:.25rem">Order #</label><input type="number" name="sort_order" id="planOrder" min="0" value="0" style="width:100%;padding:.45rem .6rem;border:1px solid var(--argon-border);border-radius:.25rem;font-size:.82rem" placeholder="0"></div>
                    ...
                    <div style="display:flex;align-items:flex-end;padding-bottom:.15rem"><label style="display:flex;align-items:center;gap:.4rem;font-size:.78rem;font-weight:600;color:var(--argon-dark);cursor:pointer"><input type="checkbox" name="is_popular" id="planPopular" style="width:auto"> Most Popular</label></div>
                </div>
...
<script>
async function loadPlans(){const r=await fetch('/api/admin/plans.php');const d=await r.json();const c=document.getElementById('plansList');if(!d.success||!d.data.plans.length){c.innerHTML='<div class="tscroll"><table><thead><tr><th>#</th><th>Name</th><th>Min</th><th>Max</th><th>Days</th><th>ROI</th><th>Status</th><th>Popular</th><th>Actions</th></tr></thead><tbody><tr><td colspan="9" style="text-align:center;padding:2rem;color:var(--argon-muted)">No plans yet. <a href="#" onclick="showCreateModal()" style="color:var(--argon-primary)">Create one</a></td></tr></tbody></table></div>';return}
c.innerHTML=`<div class="tscroll"><table><thead><tr><th>#</th><th>Name</th><th>Min</th><th>Max</th><th>Days</th><th>ROI</th><th>Status</th><th>Popular</th><th>Actions</th></tr></thead><tbody>${d.data.plans.map(p=>`<tr><td style="font-weight:700;color:var(--argon-muted)">${p.sort_order||0}</td><td style="font-weight:600;color:var(--argon-dark)">${escHtml(p.name)}</td><td>$${parseFloat(p.min_amount).toLocaleString()}</td><td>${p.max_amount? '$'+parseFloat(p.max_amount).toLocaleString():'<span style="color:var(--argon-muted)">Unlimited</span>'}</td><td>${p.duration_days}d</td><td style="font-weight:600">${parseFloat(p.daily_roi).toFixed(2)}%</td><td><a href="#" onclick="togglePlan(${p.id})" class="badge ${p.status==='active'?'b-success':'b-default'}" style="cursor:pointer;text-decoration:none">${p.status}</a></td><td>${p.is_popular==1?'<span style="color:#f59e0b;font-weight:700">★</span>':'—'}</td><td><a href="#" onclick="editPlan(${p.id},'${escHtml(p.name).replace(/'/g,"\\'")}','${escHtml(p.description||'').replace(/'/g,"\\'")}',${p.min_amount},${p.max_amount||0},${p.duration_days},${p.daily_roi},'${p.status}',${p.sort_order||0},${p.is_popular||0})" class="act-link" style="margin-right:.5rem">Edit</a></td></tr>`).join('')}</tbody></table></div>`}
...
function showCreateModal(){document.getElementById('modalTitle').textContent='Create Plan';document.getElementById('planAction').value='create';document.getElementById('planId').value='';document.getElementById('planForm').reset();document.getElementById('planStatus').value='active';document.getElementById('planOrder').value='0';document.getElementById('planPopular').checked=false;document.getElementById('planModal').style.display='flex'}
function editPlan(id,name,desc,min,max,dur,roi,status,order,popular){document.getElementById('modalTitle').textContent='Edit Plan #'+id;document.getElementById('planAction').value='update';document.getElementById('planId').value=id;document.getElementById('planName').value=name;document.getElementById('planDesc').value=desc;document.getElementById('planMin').value=min;document.getElementById('planMax').value=max||'';document.getElementById('planDuration').value=dur;document.getElementById('planRoi').value=roi;document.getElementById('planStatus').value=status;document.getElementById('planOrder').value=order||0;document.getElementById('planPopular').checked=popular==1;document.getElementById('planModal').style.display='flex'}
